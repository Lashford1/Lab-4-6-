...
def execute_go(direction):
    ...
    if is_valid_exit(current_room["exits"],direction):
        print(move(current_room["exits"],direction)["name"])
...
def execute_take(item_id):
    ...
    for item in items:
        if item["id"] == item_id:
            ...
            break
    if found:
        if (inventory_mass(inventory)+mass) <= 2.0:
            inventory.append(item)
            items.remove(item)
        else:
            print("")
            print("You're carrying too much mass. You cannot take that.")
    else:
        print("")
